...
std::string readPpmToken(std::istream& input) {
    ...
    throw std::runtime_error("Unexpected end of PPM texture file.");
}

TexturePixels loadPpmTexture(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        throw std::runtime_error("Failed to open texture file: " + filename);
    }

    const std::string magic = readPpmToken(file);
    if (magic != "P3") {
        throw std::runtime_error("Only ASCII P3 PPM textures are supported in this stage.");
    }

    TexturePixels texture;
    texture.width = static_cast<uint32_t>(std::stoul(readPpmToken(file)));
    texture.height = static_cast<uint32_t>(std::stoul(readPpmToken(file)));
    const int maxValue = std::stoi(readPpmToken(file));
    if (texture.width == 0 || texture.height == 0 || maxValue != 255) {
        throw std::runtime_error("Unsupported PPM texture dimensions or color range.");
    }

    texture.rgba.reserve(static_cast<size_t>(texture.width) * texture.height * 4);
    for (uint32_t i = 0; i < texture.width * texture.height; ++i) {
        texture.rgba.push_back(static_cast<uint8_t>(std::stoi(readPpmToken(file))));
        texture.rgba.push_back(static_cast<uint8_t>(std::stoi(readPpmToken(file))));
        texture.rgba.push_back(static_cast<uint8_t>(std::stoi(readPpmToken(file))));
        texture.rgba.push_back(255);
    }
...
ModelData loadObjModel(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (prefix == "v") {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                const size_t slash = vertexToken.find('/');
                if (slash == std::string::npos) {
                    throw std::runtime_error("Only v/vt OBJ faces are supported in this stage.");
                }

                const size_t positionIndex = static_cast<size_t>(std::stoul(vertexToken.substr(0, slash))) - 1;
                const size_t texCoordIndex = static_cast<size_t>(std::stoul(vertexToken.substr(slash + 1))) - 1;
                if (positionIndex >= positions.size() || texCoordIndex >= texCoords.size()) {
    ...
    if (model.vertices.empty() || model.indices.empty()) {
        throw std::runtime_error("Model file did not contain drawable triangles.");
    }
...
VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    // 第十五阶段开始从 OBJ 模型加载顶点和索引，模型数据转换与 Buffer 上传问题很适合交给验证层提醒。
...
private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(windowWidth, windowHeight, "Learn Vulkan - Review Refactor", nullptr, nullptr);
        if (window_ == nullptr) {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Learn Vulkan Review Refactor";
    ...
    void loadModel() {
        // 模型加载把外部文件中的位置和 UV 转换为本项目的 Vertex / index 数组。
        // 后续 GPU 只认识 Buffer，OBJ 解析结果必须先进入 staging buffer，再上传到 device local buffer。
        model_ = loadObjModel(MODEL_FILE);
    }
    ...
    void pickPhysicalDevice() {
        ...
        // 第十三阶段仍然需要交换链作为每帧呈现目标，因此设备必须支持 VK_KHR_swapchain 扩展和可用的 Surface 配置。
    ...
    void createVertexBuffer() {
        ...
        std::memcpy(data, model_.vertices.data(), static_cast<size_t>(bufferSize));
    ...
    void createIndexBuffer() {
        ...
        std::memcpy(data, model_.indices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(device_, stagingBufferMemory);
    ...
    void createTextureImage() {
        const TexturePixels texture = loadPpmTexture(TEXTURE_FILE);
        const VkDeviceSize imageSize = texture.rgba.size();
        ...
        void* data = nullptr;
        check(vkMapMemory(device_, stagingBufferMemory, 0, imageSize, 0, &data), "Failed to map texture staging buffer memory.");
        std::memcpy(data, texture.rgba.data(), static_cast<size_t>(imageSize));
        vkUnmapMemory(device_, stagingBufferMemory);
        ...
        // Vulkan Image 必须处于匹配用途的布局中：先转为传输目标，拷贝像素后再转为 shader 只读。
        ...
        copyBufferToImage(stagingBuffer, textureImage_, texture.width, texture.height);
    ...
    void createTextureSampler() {
        ...
        check(vkCreateSampler(device_, &samplerInfo, nullptr, &textureSampler_), "Failed to create texture sampler.");
    }

    void createDepthResources() {
        const VkFormat depthFormat = findDepthFormat();

        // 深度图像和交换链图像尺寸一致，每个像素保存一个深度值。
        // 它不需要被 shader 采样，本阶段只作为 Render Pass 的 depth attachment 使用。
        createImage(
    ...
    void createDescriptorPool() {
        ...
        // 第十三阶段同时需要 Uniform Buffer 和 Combined Image Sampler 两类描述符。
    ...
    void createRenderPass() {
        ...
        // 当前只需要一个图形子通道，把颜色写入第 0 个附件，并把深度写入第 1 个附件。
        ...
        // 这里建立颜色和深度附件写入所需的基础依赖，帧级同步由 Semaphore 和 Fence 负责。
    ...
    void createGraphicsPipeline() {
        ...
        // 第十五阶段继续使用 Uniform Buffer 和纹理采样器的 Descriptor Set Layout。
    ...
    void createCommandBuffers() {
        ...
        for (size_t i = 0; i < commandBuffers_.size(); ++i) {
            ...
            vkCmdDrawIndexed(commandBuffers_[i], static_cast<uint32_t>(model_.indices.size()), 1, 0, 0, 0);
    ...
    void printSelectedDeviceInfo() const {
        ...
        std::cout << "Model vertices: " << model_.vertices.size() << '\n';
        std::cout << "Model indices: " << model_.indices.size() << '\n';
        std::cout << "Uniform buffer and descriptor set: created" << '\n';
        std::cout << "Texture image and sampler: created" << '\n';
