...
VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    // 第九阶段开始真正提交绘制命令，同步对象和队列提交顺序的问题很适合交给验证层提醒。
...
private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(windowWidth, windowHeight, "Learn Vulkan - Sync Render Loop", nullptr, nullptr);
        if (window_ == nullptr) {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Learn Vulkan Sync Render Loop";
    ...
    void pickPhysicalDevice() {
        ...
        // 第九阶段需要交换链作为每帧呈现目标，因此设备必须支持 VK_KHR_swapchain 扩展和可用的 Surface 配置。
