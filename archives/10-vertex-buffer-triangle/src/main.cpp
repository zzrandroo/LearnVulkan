...
VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    // 第十阶段开始使用 Vertex Buffer，缓冲创建、内存绑定和顶点输入配置的问题很适合交给验证层提醒。
...
private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(windowWidth, windowHeight, "Learn Vulkan - Vertex Buffer Triangle", nullptr, nullptr);
        if (window_ == nullptr) {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Learn Vulkan Vertex Buffer Triangle";
    ...
    void pickPhysicalDevice() {
        ...
        // 第十阶段仍然需要交换链作为每帧呈现目标，因此设备必须支持 VK_KHR_swapchain 扩展和可用的 Surface 配置。
    ...
    void createVertexBuffer() {
        ...
        // 第十阶段为了把重点放在 Vertex Buffer 流程上，直接使用 CPU 可见的内存上传顶点。
        // HOST_COHERENT 可以避免手动 flush；更高性能的 DEVICE_LOCAL + Staging Buffer 会在下一阶段学习。
        createBuffer(
            bufferSize,
        ...
        void* data = nullptr;
        check(vkMapMemory(device_, vertexBufferMemory_, 0, bufferSize, 0, &data), "Failed to map vertex buffer memory.");
    ...
    void createCommandBuffers() {
        ...
        for (size_t i = 0; i < commandBuffers_.size(); ++i) {
            ...
            // 顶点数据现在来自 Vertex Buffer，vkCmdDraw 的顶点数量必须和缓冲中的顶点数量一致。
            // 这条命令会按绑定的顶点布局读取 position 和 color，再交给 vertex shader。
