...
VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    // 第六阶段开始创建 Image View 和 Render Pass，附件布局和对象销毁顺序的问题很适合交给验证层提醒。
...
private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(windowWidth, windowHeight, "Learn Vulkan - Image View Render Pass", nullptr, nullptr);
        if (window_ == nullptr) {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Learn Vulkan Image View Render Pass";
    ...
    void pickPhysicalDevice() {
        ...
        // 第六阶段沿用交换链筛选条件：设备必须支持 VK_KHR_swapchain 扩展和可用的 Surface 配置。
    ...
    void mainLoop() const {
        // 现在已经有 Image View 和 Render Pass，但还没有 Framebuffer 和命令录制。
