...
VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    // 第八阶段开始创建 Framebuffer 和录制 Command Buffer，渲染通道范围和命令录制顺序的问题很适合交给验证层提醒。
...
private:
    ...
    void initWindow() {
        ...
        window_ = glfwCreateWindow(windowWidth, windowHeight, "Learn Vulkan - Framebuffer Command Buffer", nullptr, nullptr);
        if (window_ == nullptr) {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Learn Vulkan Framebuffer Command Buffer";
    ...
    void pickPhysicalDevice() {
        ...
        // 第八阶段仍然需要交换链作为命令缓冲的渲染目标，因此设备必须支持 VK_KHR_swapchain 扩展和可用的 Surface 配置。
    ...
    void createCommandBuffers() {
        ...
        for (size_t i = 0; i < commandBuffers_.size(); ++i) {
            ...
            // 顶点 shader 使用 gl_VertexIndex 生成三个顶点，所以这里直接绘制 3 个顶点。
            // 这条命令此刻只是被写入 Command Buffer，真正执行要等后续阶段提交到图形队列。
    ...
    void mainLoop() const {
        // 现在已经录制了 Command Buffer，但还没有同步对象和 draw frame 流程。
        // 窗口循环仍然只负责处理事件，真正提交队列的绘制流程会在下一阶段加入。
